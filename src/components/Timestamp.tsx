...
export function timestampSVG(
  ...
  if (timeStart && timeEnd && progressBar)
    return renderProgressBar(timeStart, timeEnd, x, colors, y);
...
function renderText(
  ...
  const animatedDurationSVG = animatedText(
    durationValues,
    `style="fill: ${statusColors.online}; font-family:${fontFamily}; font-size: 20px;font-weight:600;" x="${x + 25}" y="${y + 30}"`
  );
  return `<g>
    ${controllerPath}
    ${animatedDurationSVG}
...
function renderProgressBar(
  ...
  const progress = Math.min(
    (Date.now() - timeStart) / (timeEnd - timeStart),
    1
  );
  const timeElapsed = timeStart ? prettyDuration(Date.now() - timeStart) : "";
  const barWidth = 700 - x - 140;
  return `<g>
    <text style="fill: ${colors.colorT2}; font-family:${fontFamily}; font-size: 20px;" x="${x}" y="${y + 30}">
      ${Date.now() > timeEnd ? totalActivityLength : timeElapsed}
    </text>
    <rect x="${x + 50}" y="${y + 22}" width="${barWidth}" height="6" rx="3" style="fill:${colors.colorB3};"/>
    <rect x="${x + 50}" y="${y + 22}" width="${barWidth * progress}" height="6" rx="3" style="fill:${colors.colorT1};"/>
    <text style="fill: ${colors.colorT2}; font-family:${fontFamily}; font-size: 20px;" x="${x + barWidth + 60}" y="${y + 30}">
      ${totalActivityLength}
    </text>
  </g>`;
}
