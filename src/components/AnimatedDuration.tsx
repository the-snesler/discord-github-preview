  ...
  return strings
    .map((str, index) => {
      ...
      if (index === 0) {
        ...
        animations =
          `<set attributeName="opacity" to="1" begin="${index}s" fill="freeze" />` +
          (index < strings.length - 1
            ? `<set attributeName="opacity" to="0" begin="${index + 1}s" fill="freeze" />`
            : "");
      }
      return `<text x="${x}" y="${y}" font-family="${fontFamily}" font-size="14" fill="${colors.colorT2}" opacity="${index === 0 ? 1 : 0}">${str}${animations}</text>`;
    })
    .join("");
}
