...
export const richPresence: ActivityDisplay = {
  ...
  render: async function (activity: Activity, colors: ColorTheme, y: number, width: number): Promise<string> {
    ...
    return `<g>
      ...
      <text style="fill:${colors.colorT2};font-family:${fontFamily};font-size:24px;font-weight:600;" x="${textX}" y="${textY}">${sanitizeString(activity.name)}</text>
      ${detailLines.map((line, i) => `<text style="fill: ${colors.colorT2}; font-family:${fontFamily}; font-size: 20px;" x="${textX}" y="${textY + 30 + (i * 30)}">${sanitizeString(line || "")}</text>`).join("")}
