...
export const spotifyActivity: ActivityDisplay = {
  ...
  render: async function (activity: Activity, colors: ColorTheme, y: number): Promise<string> {
    ...
    return `<g>
      ...
      <text style="fill:${colors.colorT1};font-family:${fontFamily};font-size:22px;font-weight:600;white-space:pre;" x="${textX}" y="${textY}">
        ${sanitizeString(songName)}
      </text>
      <text style="fill:${colors.colorT2};font-family:${fontFamily};font-size:18px;white-space:pre;" x="${textX}" y="${textY + 30}">
        by ${sanitizeString(artistName)}
      </text>
      <text style="fill:${colors.colorT2};font-family:${fontFamily};font-size:16px;white-space:pre;font-style:italic;" x="${textX}" y="${textY + 52}">
