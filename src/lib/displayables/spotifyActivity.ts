...
import { ActivityDisplay, ColorTheme } from "../..";
...
export const spotifyActivity: ActivityDisplay = {
  ...
  render: async function (activity: Activity, colors: ColorTheme, y: number): Promise<string> {
    ...
    if (hasTimestamps) {
      ...
      progressBar = `
				<rect x="${textX}" y="${textY + 65}" width="490" height="4" rx="2" style="fill:${colors.colorB3};"/>
    ...
    return `<g>
      ...
      <rect x="${smallCenter[0] - 22}" y="${y + smallCenter[1] - 22}" width="44" height="44" rx="100%" style="fill:${colors.colorB2};"/>
      <circle cx="${smallCenter[0]}" cy="${y + smallCenter[1]}" r="19" fill="${spotifyGreen}"/>
      <path transform="translate(${smallCenter[0]} ${y + smallCenter[1]}) scale(3.5) translate(-41.8 -1.5)" d="M44.9 1c-1.6-1-4.3-1.1-5.8-0.6-0.3 0.1-0.5-0.1-0.6-0.3-0.1-0.3 0.1-0.5 0.3-0.6 1.8-0.5 4.7-0.4 6.6 0.7 0.2 0.1 0.3 0.4 0.2 0.6-0.1 0.2-0.4 0.3-0.7 0.2zm-0.3 1.5c-0.1 0.2-0.4 0.3-0.6 0.1-1.3-0.8-3.4-1.1-4.9-0.6-0.2 0-0.5 0-0.5-0.2-0.1-0.2 0-0.5 0.2-0.5 1.8-0.6 4.2-0.3 5.8 0.7 0.1 0.1 0.1 0.4 0 0.5zm-0.7 1.5c-0.1 0.1-0.3 0.2-0.5 0.1-1.2-0.7-2.6-0.9-4.3-0.5-0.2 0-0.3-0.1-0.4-0.2-0.1-0.2 0.1-0.4 0.2-0.4 1.9-0.4 3.6-0.2 4.9 0.6 0.3 0.1 0.3 0.3 0.1 0.4z" fill="${colors.colorB1}"/>
      <text style="fill:${colors.colorT1};font-family:${FONT_FAMILY};font-size:22px;font-weight:600;white-space:pre;" x="${textX}" y="${textY}">
        ${sanitizeString(songName)}
      </text>
      <text style="fill:${colors.colorT2};font-family:${FONT_FAMILY};font-size:18px;white-space:pre;" x="${textX}" y="${textY + 30}">
        by ${sanitizeString(artistName)}
      </text>
      <text style="fill:${colors.colorT2};font-family:${FONT_FAMILY};font-size:16px;white-space:pre;font-style:italic;" x="${textX}" y="${textY + 52}">
