...
export const genericActivity: ActivityDisplay = {
  ...
  render: async function (activity: Activity, colors: ColorTheme, y: number): Promise<string> {
    ...
    return `<g>
      ...
      <text style="fill:${colors.colorT2};font-family:${FONT_FAMILY};font-size:24px;font-weight:600;" x="${textX}" y="${textY}">${activity.name}</text>
      ${iconURL ?
        `<image xlink:href="${await URItoBase64(iconURL)}" x="30" y="${y + 10}" height="120" width="120" clip-path="inset(0% round 5px)" />`
        : placeholderPath}
      ${timestampSVG(textX, textY, colors, activity.timestamps?.start?.getTime() || activity.createdTimestamp)}
    </g>`;
  }
};
