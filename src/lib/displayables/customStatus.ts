...
export const customStatus: ActivityDisplay = {
  ...
  render: async function (activity: Activity, colors: ColorTheme, y: number): Promise<string> {
    ...
    ${hasCustomEmoji ?
        ...
        hasEmoji ?
          `<text style="fill: ${colors.colorT1}; font-family:${FONT_FAMILY}; font-size:${emojiSize - 4}px;" x="220" y="${bannerHeight + emojiSize + 10}">${emojiName}</text>`
          : ""}
    ${hasText ? `<foreignObject x="${xOffset}" y="${bannerHeight + 20}" width="${700 - xOffset - 40}" height="60">
      <p xmlns="http://www.w3.org/1999/xhtml" 
      style="color: ${colors.colorT2}; margin: 0; font-family:${FONT_FAMILY}; font-size: 22px; font-style: italic; line-height: 1.2em;">
      ${activity.state}</p>
    </foreignObject>` : ""}
  </g>`;
  }
