...
function renderText(timeStart: number | null, timeEnd: number | null, x: number, y: number) {
  ...
  const animatedDurationSVG = animatedText(durationValues, `style="fill: ${statusColors.online}; font-family:${FONT_FAMILY}; font-size: 20px;font-weight:600;" x="${x + 25}" y="${y + 30}"`);
  return `<g>
    ${controllerPath}
    ${animatedDurationSVG}
  </g>`;
...
function renderProgressBar(timeStart: number, timeEnd: number, x: number, colors: ColorTheme, y: number) {
  ...
  return `<g>
    <text style="fill: ${colors.colorT2}; font-family:${FONT_FAMILY}; font-size: 20px;" x="${x}" y="${y + 30}">
    ...
    <text style="fill: ${colors.colorT2}; font-family:${FONT_FAMILY}; font-size: 20px;" x="${x + barWidth + 60}" y="${y + 30}">
      ${totalActivityLength}
    </text>
  </g>`;
}
